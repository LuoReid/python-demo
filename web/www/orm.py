...
class StringField(Field):
    def __init__(self, name=None, primary_key=False, default=None, ddl='varchar(64)'):
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    async def update(self):
        ...
        if rs != 1:
            logging.warn(
                'failed to update by primary key: affected rows: %s' % rs)
    ...
    async def remove(self):
        ...
        if rs != 1:
            logging.warn(
                'failed to remove by primary key: affected rows: %s' % rs)
